...
impl<'a, T: Authenticator + 'a> CDRS<T> {
    ...
    pub fn start(mut self, compressor: Compression) -> error::Result<Session<T>> {
        ...
        if start_response.opcode == Opcode::Authenticate {
            ...
            let authenticator = body.get_authenticator().expect("Cassandra Server did communicate
            that it needed password authentication but the  auth schema was missing in the body response");
